...
const callOf = (tool, args) =>
  `${tool}(${Object.entries(args ?? {}).map(([name, value]) => `${name}=${brief(value, 60)}`).join(', ')})`

/** Iterations out of a ResumableReAct trajectory, oldest first. */
export function iterations(payload) {
  ...
  for (let i = 0; trajectory[`thought_${i}`] !== undefined; i += 1) {
    ...
    rows.push({
      ...
      observation,
      // The proposed step is in the trajectory already; only its observation is
      // missing, because it has not run.
...
const failed = (observation) => typeof observation === 'string' && observation.startsWith('Execution error in')

/**
 * How the agent got here. Finished iterations collapse to their tool call,
 * which is what identifies them; only the unanswered one stays open.
 */
export function Trace({ payload }) {
  ...
  return (
    <div class="trace">
      <button type="button" class="trace-row" style="border-bottom: 0;" onClick={() => setOpen(!open)}>
        <Toggle open={open} />
        <span style="font-size: 12.5px;">How it got here</span>
        <span class="mono" style="font-size: 11px; color: var(--dim);">{rows.length} steps</span>
      </button>
      {open ? rows.map((row) => (row.pending ? <Pending key={row.index} row={row} /> : <Finished key={row.index} row={row} />)) : null}
    </div>
  )
}
    ...
    <>
      ...
      {open ? (
        <div style="padding: 4px 0 10px 26px;">
          <div style="font-family: var(--serif); font-size: 11.5px; color: #8a867e;">{row.thought}</div>
          <div class={`obs ${bad ? 'bad' : ''}`} style={bad ? 'color: var(--red);' : ''}>{brief(row.observation)}</div>
        </div>
      ) : null}
    </>
  )
