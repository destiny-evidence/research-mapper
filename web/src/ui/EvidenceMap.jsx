...
const Bubble = ({ n }) => {
  ...
  return (
    <span class="bubble" style={`width: ${size}px; height: ${size}px; font-size: ${font}px;`}>
      {n}
    </span>
  )
}
...
const Axis = ({ label, value, options, onChange }) => (
  <label class="axis">
    ...
    <select class="field" value={String(value)} onInput={(event) => onChange(Number(event.currentTarget.value))}>
      {options.map((dimension, index) => (
        <option key={dimension.name} value={String(index)}>{dimension.name}</option>
      ))}
    </select>
  </label>
)
...
export function EvidenceMap({ map, included }) {
  ...
  const pick = (which) => (index) =>
    setAxes(({ row, col }) => {
      ...
      setFacet(null)
      return which === 'row' ? { row: index, col: swapped } : { row: swapped, col: index }
    })
    ...
    <>
      <div class="map-head">
        ...
        <div style="font-size: 12px; color: var(--muted); margin-top: 5px;">
          {grid.placed} placed{unplaced > 0 ? `. ${unplaced} could not be placed.` : '.'}
        </div>
      </div>

      <div class="map-controls">
        <Axis label="Rows" value={grid.rowIndex} options={map.dimensions} onChange={pick('row')} />
        <Axis label="Columns" value={grid.colIndex} options={map.dimensions} onChange={pick('col')} />
        <div class="facets">
          <span class="lab">{grid.facetDim.name}</span>
          <button type="button" class={`facet ${facet ? '' : 'on'}`} onClick={() => setFacet(null)}>
            <span>All</span>
          ...
          {grid.facets.map((option) => (
            ...
            >
              <span><Breakable>{option.name}</Breakable></span>
